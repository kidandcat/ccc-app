...
A machine opens on **General**, the dispatcher — same as the Telegram DM. Workers General starts show as live status cards (on General and under Sessions) with running / waiting / idle, and drop off when archived. Unanswered `ask_owner` questions stay in a Decisions inbox and a sticky card above chat — same option buttons as Telegram.
